...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function performCalculation(data) {
        try {
            ...
            const basicSalary = data.calculationType === 'backward' ? result.basic : data.basicSalary;
            const netSalary = data.calculationType === 'forward' ? result.net : data.netSalary;
    ...
    function formatCurrency(amount) {
        ...
        return 'R' + amount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '// ZA Tax Calculator - JavaScript functionality
document.addEventListener('DOMContentLoaded', function() {
